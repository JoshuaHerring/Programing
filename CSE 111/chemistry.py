def main():
    choice = input('Would you like to select a know molecule? y/n ')
    if choice == 'y':
        molecule = input('What is the name of your molecule? ')
        formula = known_molecules(molecule)
    else:
    # Get a chemical formula for a molecule from the user.
        formula = input('What is the chemical formula for your molecule? ')

    # Get the mass of a chemical sample in grams from the user.
    mass_in_grams = float(input('What is the mass in grams of your smaple? '))
    ...
    print(f'Grams per mole: {molar_mass:.8}')
    # Print the number of moles.
    print (f'Number of Moles: {number_of_moles:.4}')
...
def compute_molar_mass(symbol_quantity_list, periodic_table_dict):
    ...
    length = len(symbol_quantity_list)
    total_mass = 0
    for x in range(length):
        element = symbol_quantity_list[x]
        ...
        select_mass = atomic_mass * quantity
        total_mass = total_mass + select_mass
        # Add the product into the total mass.

    # Return the total mass.
    return total_mass

def known_molecules(molecule):
        ...
        formula = known_molecules_dict[molecule]
        return formula
#################################################################
